    ...
    if (name) {
      ...
      try {
        ...
        normalizePokemonDb.push({
          ...
          types: nameQuery[0]?.dataValues.types.map((n) => {
            return { name: n.name };
          }),
      ...
      try {
        ...
        for (var i = 0; i < include.length; i++) {
          normalize.push({
            ...
            types: include[i].dataValues.types.map((n) => {
              return { name: n.name };
            }),
        ...
        let subrequest = apiLink.map((e) => axios.get(e.url));
    ...
    if (id.length < 5) {
      ...
      try {
        ...
        normalizePokemonIdDb.push({
          ...
          types: idParams?.dataValues.types,
        });
  ...
  router.post("/", async (req, res, next) => {
    try {
      ...
      let newPokemon = await Pokemon.create({
        name, image, hp, attack, defense, speed, height, weight, image});
  
      let typesFind = await Type.findAll({
        where: { name: types },
      });
  
      newPokemon.addTypes(typesFind);
      res.send(newPokemon);
    } catch (error) {
      next(error);
    }
